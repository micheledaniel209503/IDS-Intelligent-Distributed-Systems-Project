function rho = ring2d(X, Y, mu, R, sigma_r, eps_bg)
    ...
    % Set default value for background noise if not provided
    if nargin < 6 || isempty(eps_bg), eps_bg = 1e-3; end
    ...
    rho = eps_bg + exp(- (rr - R).^2 / (2*sigma_r^2));   % non normalizzata (va benissimo)
end
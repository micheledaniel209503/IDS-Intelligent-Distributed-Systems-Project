function Wrs = build_Wrs_fast_ray2(pr, Vmask, obs, X, Y, rs, ds)
...
% ds     = step size along radius
    ...
    % precompute conversion world coordinates --> indices of the grid
    % ix = round((x - xv(1))/dx) + 1
    invdx = 1/dx; 
    invdy = 1/dy; 
    x0 = xv(1); 
    y0 = yv(1);
    ...
    % number of steps on the scanned radius (scan all the radius up to rs,
    % with step size = ds)
    nsteps = floor(rs/ds);
    ...
    for t = 1:numel(ang) % loop through each angle
        ...
        for s = 0:nsteps
            ...
            % bounds check (point is outside map boundaries)
            if jx < 1 || jx > n || iy_int < 1 || iy_int > m
                break; % stop
            end

            % found an obstacle on the point
            if obs(iy_int, jx)
                break;
            end
    ...
    % disk intersection (just for safety, not necessary if the loop is on rs)
    Rmask = (X - pr(1)).^2 + (Y - pr(2)).^2 <= rs^2;

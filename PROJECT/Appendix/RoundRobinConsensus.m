...
Packages(Npack,1) = pkgg();
for k = 1:Npack
    Packages(k).id = k;
    [Packages(k).x, Packages(k).y] = spawn_pkgg(zones.inbound);
...
disp(['Qtot matrix after on entire round of message exchanged']);
disp(Q_tot);
disp(['Qtot^15']);
disp(Q_tot^15);
